...
* **Выполнимость**. Высказывание выполнимо, если существует оценка, в которой оно истинно.
* **Невыполнимость.** Высказывание невыполнимо, если нет оценок, в которых оно истинно.
* **Опровержимость**. Высказывание опровержимо, если существует оценка, в которой оно ложно.
...
* Индукция по длине вывода **n**. Для каждого высказывания $\delta_n$ из вывода разбор случаев:
  ...
  2. Modus Ponens j,k - убедится, что если $\vDash \delta_j$ и $\vDash \delta_j\rightarrow\delta_n$ , то $\vDash\delta_n$.

  Итак, приступим.

* *Для начала разберем **схесмы аксиом.***

* ** Строим таблицы истинности для всех аксиом, и убеждаемся в их общезначимости**

  * Например, для аксиомы 9

  | $[\alpha]$ | $[\beta]$ | ...  | $[\alpha\rightarrow\neg\beta]$ | $[(\alpha\rightarrow\neg\beta)\rightarrow\neg\alpha]$ | $[(\alpha\rightarrow\beta)\rightarrow(\alpha\rightarrow\neg\beta)\rightarrow\neg\alpha]$ |
  | :--------: | :-------: | :--: | :----------------------------: | :---------------------------------------------------: | :----------------------------------------------------------: |
  |     Л      |     Л     |      |               И                |                           И                           |                              И                               |
  |     Л      |     И     |      |               И                |                           И                           |                              И                               |
  |     И      |     Л     |      |               И                |                           Л                           |                              И                               |
...
* Общезначимость **Modens Ponens**

  * Пусть в выводе есть формулы $\delta_j,\delta_k=\delta_j\rightarrow\delta_n, \delta_n$(причем, j < n и k < n)

  * Зафиксируем какую-нибудь оценку. По индукционному предположению, $\delta_j$ и $\delta_j\rightarrow\delta_n$ общезначимы. Поэтому в этой оценке они оцениваются в И и И.

  * Построим таблицу истинности для импликации:

    * | $[\delta_j]$ | $[\delta_n]$ | $[\delta_j\rightarrow\delta_n]$ |
      | :----------: | :----------: | :-----------------------------: |
      |      Л       |      Л       |                И                |
      |      Л       |      И       |                И                |
      |      И       |      Л       |                Л                |
    ...
    *  Из этой таблицы видно, что $\delta_n$ может оказаться ложной только если $\delta_j$ ложная, либо $\delta_j\rightarrow\delta_n$ ложна. Если оба истинны, то dn обязанна быть истинной. Значит $\delta_n$ оценивается в истину.

    * **Теорема доказана;**

  * **Таким образом,** если мы доказали какую-то формулу, то она обязательно будет общезначимой.</details>
